...
export default function CreateStore() {
    ...
    try {
      ...
      if (["pending", "approved", "rejected"].includes(data.status)) {
        ...
        switch (data.status) {
          case "approved":
            setMessage(
              "Your store has been approved, you can now add products and start selling from your dashboard. Redirecting you to dashboard..."
            );
            setTimeout(() => {
              router.push("/store");
            }, 5000);
          ...
          case "rejected":
            setMessage(
              "Unfortunately, your store application was rejected. Please review the guidelines and consider reapplying."
            );
            break;
          default:
            break;
      ...
      toast.error(
        `An error occurred while fetching seller status: ${
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      toast.success(`Store submitted successfully! Status: ${data.status}`);
      await fetchSellerStatus();
    } catch (error) {
      console.error("Error submitting store data:", error);
      toast.error(
        `An error occurred while submitting store data: ${error.message}`
      );
      throw error;
    }
  };
  ...
  if (!user) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center">
        <p className="text-base mx-5 font-semibold text-slate-500 text-center max-w-2xl">
          ...
          >
            ...
            <div>
              ...
              <p className="max-w-lg">
                To become a seller on Aura, submit your store details for
