...
export const initiateStkPush = async ({
  ...
  // Password + BusinessShortCode always use the shortcode (a Paybill number, or the
  // Store/Head-Office number for a Buy Goods Till). Timestamp binds the password.
  ...
  // If a Till number is configured, this is a Buy Goods (Till) account:
  // PartyB = Till number, TransactionType = CustomerBuyGoodsOnline.
  // Otherwise it's a Paybill: PartyB = the shortcode.
  const till = process.env.MPESA_TILL_NUMBER;
  const transactionType = till
    ? "CustomerBuyGoodsOnline"
    : "CustomerPayBillOnline";
  const partyB = till || shortcode;
